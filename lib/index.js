...
function getDefaultConfig() {
    return {
        ...
        server: { port: 3000, logHttpRequests: false },
        types: {
            doc: { view: 'document', title: 'Document' }
        },
        menu: [
...
function getInitConfig() {
    return _.merge(getDefaultConfig(), {
        generators: {
            typedoc: {
...
function relPathToDestRoot(dir) {
    var rootPath = path.relative(dir, destPath());
    rootPath = rootPath.length > 0 ? rootPath + '/' : '';
    return rootPath;
}
exports.relPathToDestRoot = relPathToDestRoot;
